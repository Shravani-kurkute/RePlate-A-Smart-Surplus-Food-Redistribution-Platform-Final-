...
router.post('/register/seller', async (req, res) => {
    try {
        const { 
            businessName, ownerName, businessType, gstNumber, fssaiNumber,
            email, phone, address, city, pincode, password 
        } = req.body;
        
        // Validation
        if (!businessName || !ownerName || !businessType || !email || 
            !phone || !address || !city || !pincode || !password) {
            return res.status(400).json({
                success: false,
        ...
        const [userResult] = await pool.query(
            `INSERT INTO users (full_name, email, phone, city, pincode, password_hash, user_role, is_verified)
             VALUES (?, ?, ?, ?, ?, ?, 'seller', FALSE)`,
            [ownerName, email, phone, city, pincode, passwordHash]
        );
        
        const userId = userResult.insertId;
        
        // Insert restaurant/business details
        const [restaurantResult] = await pool.query(
            `INSERT INTO restaurants 
             (user_id, restaurant_name, cuisine_type, address, city, pincode, phone, is_verified, is_active)
             VALUES (?, ?, ?, ?, ?, ?, ?, FALSE, FALSE)`,
            [userId, businessName, businessType, address, city, pincode, phone]
        );
        
        const restaurantId = restaurantResult.insertId;
        
        // Store GST and FSSAI if provided
        if (gstNumber || fssaiNumber) {
            await pool.query(
                `UPDATE restaurants 
                 SET business_license = ?, description = ?
                 WHERE restaurant_id = ?`,
                [gstNumber || null, `FSSAI: ${fssaiNumber || 'N/A'}`, restaurantId]
            );
        }
        
        // Send notification to admin for approval
        await sendAdminNotification(businessName, ownerName, email);
        
        // Send confirmation email to seller
        await sendSellerConfirmationEmail(email, ownerName, businessName);
